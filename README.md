...
if df is not None:
    ...
    # ดึงรายชื่อคอลัมน์ที่เป็นไปได้สำหรับค้นหาวันที่
    date_cols = [col for col in df.columns if 'นัด' in col or 'คอลัมน์ 1' in col or 'วันที่' in col]
    if not date_cols:
    ...
    with col1:
        target_col = st.selectbox("📌 เลือกคอลัมน์ที่ต้องการกรองวันที่:", options=date_cols, index=date_cols.index('นัด') if 'นัด' in date_cols else 0)
